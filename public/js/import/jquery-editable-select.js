...
(function($) {
    ...
    $.fn.editableSelect = function(options) {
        ...
        $(this).each(function(){
            ...
            + '<div class="edit_selected"><span></span>'
            ...
            var _opt_click = function() {
                ...
                $selectWrap.find(".edit_selected").attr("data-val",val).find('span').text(txt);
                $selectWrap.find(".edit_selected").find('i').removeClass('fa-angle-up').addClass('fa-angle-down');
                $select.val(val);
                $downbox.hide();
                $(this).addClass('over edit_selected').siblings().removeClass('over edit_selected');
            };
            var _new_opt = function(txt, val, isSelected) {
                var $opt = $("<li>").text(txt).attr("data-val",val);
                if(isSelected) {
                    $selectWrap.find(".edit_selected").attr("data-val",val).find('span').text(txt);
                    $opt.addClass('over').addClass('edit_selected');
                }
            ...
            var _show = function(bShow) {
                if(bShow) {
                    ...
                    if(!$options.find(".edit_selected").hasClass('over')) {
                        $options.find(".edit_selected").addClass('over').siblings().removeClass('over');
                    }
                    $downbox.show();
                } else {
            ...
            $selectWrap.find(".edit_selected").click(function() {
